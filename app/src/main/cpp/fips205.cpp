...
uint128_t uint128_t::operator*(uint64_t rhs) const {
...
ByteVector uint128_t::toByteVector(size_t n) const {
    ...
    ByteVector result(n, 0);

    // Convertir a array de 16 bytes completo primero
    uint8_t full_bytes[16];

    // high_ (bits 127-64): bytes 0-7 en big-endian
    for (int i = 0; i < 8; ++i) {
        full_bytes[i] = static_cast<uint8_t>((high_ >> (8 * (7 - i))) & 0xFF);
    }

    // low_ (bits 63-0): bytes 8-15 en big-endian
    for (int i = 0; i < 8; ++i) {
        full_bytes[8 + i] = static_cast<uint8_t>((low_ >> (8 * (7 - i))) & 0xFF);
    }

    // Copiar solo los n bytes solicitados (desde el final para valores pequeños)
    size_t start_offset = (n < 16) ? (16 - n) : 0;
    std::copy(full_bytes + start_offset, full_bytes + start_offset + n, result.begin());

    return result;
}

uint128_t uint128_t::mask_96bits() const {
...
uint128_t uint128_t::fromByteVector(const ByteVector& bytes, size_t offset, size_t length) {
    ...
    if (length > 16) {
        length = 16; // Truncar si es demasiado largo
    }

    uint128_t result;

    // Leer bytes en orden big-endian
    for (size_t i = 0; i < length; ++i) {
        result = (result << 8) + uint128_t(static_cast<uint64_t>(bytes[offset + i]));
    }
...
uint128_t uint128_t::create_mask(int bits) {
...
uint128_t uint128_t::create_96bit_mask() {
...
uint128_t toInt128_TreeAddress(const ByteVector& X, uint64_t n) {
    if (n > 12) n = 12;  // Máximo 12 bytes = 96 bits

    if (X.size() < n || n == 0) {
        return uint128_t();
    }

    // Leer exactamente n bytes en big-endian
    uint128_t result;
    for (size_t i = 0; i < n; ++i) {
        result = (result << 8) + uint128_t(static_cast<uint64_t>(X[i]));
    }

    // Aplicar máscara de 96 bits por seguridad
    return result.mask_96bits();
}
...
void ADRS::setTreeAddress(const uint128_t& tree) {
    // Aplicar máscara de 96 bits para asegurar que solo usamos 12 bytes
    uint128_t masked_tree = tree.mask_96bits();

    // Convertir a exactamente 12 bytes en big-endian
    ByteVector tree_bytes = masked_tree.toByteVector(12);

    // Copiar los 12 bytes a las posiciones 4-15 del ADRS
    std::copy(tree_bytes.begin(), tree_bytes.end(), addr.begin() + 4);
}


void ADRS::setTreeAddress(uint64_t tree) {
    // Convertir uint64_t a uint128_t y llamar a la versión principal
    setTreeAddress(uint128_t(tree));
}
...
ByteVector ht_sign(const ByteVector& M, const ByteVector& SKseed, const ByteVector& PKseed,
                   const uint128_t& idx_tree_128, uint64_t idx_leaf) {
    const SLH_DSA_Params* params = FIPS205ConfigManager::getCurrentParams();
    if (!params) {
        throw std::runtime_error("SLH_DSA_Params not initialized.");
    }
...
bool ht_verify(const ByteVector& M, const ByteVector& SIG_HT, const ByteVector& PKseed,
               const uint128_t& idx_tree_128, uint64_t idx_leaf, const ByteVector& PKroot) {
    const SLH_DSA_Params* params = FIPS205ConfigManager::getCurrentParams();
    if (!params) {
        throw std::runtime_error("SLH_DSA_Params not initialized.");
    }
    ...
    for (uint64_t j = 1; j < d; j++) {
        ...
        // Nota: (1 << h_prima) - 1 crea una máscara para h_prima bits
        uint128_t mask = uint128_t::create_mask(h_prima);
        uint64_t idx_leaf_j = static_cast<uint64_t>(current_tree & mask);
...
SLH_DSA_Signature slh_sign_internal(const ByteVector& M,
    ...
    // Aplicar máscaras con precisión completa
    ...
    // Para leaf_idx, convertir a uint64_t (debería caber siempre)
...
bool slh_verify_internal(const ByteVector& M, const ByteVector& SIG, const SLH_DSA_PublicKey& PK) {
    ...
    uint64_t idx_leaf = static_cast<uint64_t>(idx_leaf_128);  // Solo convertir leaf
    adrs.setKeyPairAddress(idx_leaf);
