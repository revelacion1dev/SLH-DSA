...
public:
    ...
    // ✅ GETTERS INLINE - PERFECTO
    uint64_t low() const { return low_; }
    uint64_t high() const { return high_; }

    // ✅ OPERADORES INLINE - EXCELENTE PARA RENDIMIENTO
    ...
    // ✅ OPERADORES COMPLEJOS: SOLO DECLARACIÓN EN HEADER
    uint128_t operator*(const uint128_t& other) const;
    uint128_t operator*(uint64_t rhs) const;
    uint128_t operator<<(int shift) const;
    uint128_t operator>>(int shift) const;

    // ✅ MÉTODOS COMPLEJOS: SOLO DECLARACIÓN EN HEADER
    ByteVector toByteVector(size_t n) const;
    uint128_t mask_96bits() const;

    // ✅ MÉTODOS ESTÁTICOS: SOLO DECLARACIÓN EN HEADER
    static uint128_t fromByteVector(const ByteVector& bytes, size_t offset = 0, size_t length = 12);
    static uint128_t create_mask(int bits);
    static uint128_t create_96bit_mask();
...
public:
    ...
    void setTreeAddress(const uint128_t& tree);      // Versión principal
    void setTreeAddress(uint64_t tree);              // Versión para uint64_t
...
ByteVector ht_sign(const ByteVector& M, const ByteVector& SKseed, const ByteVector& PKseed,
                   const uint128_t& idx_tree, uint64_t idx_leaf);

bool ht_verify(const ByteVector& M, const ByteVector& SIG_HT, const ByteVector& PKseed,
               const uint128_t& idx_tree, uint64_t idx_leaf, const ByteVector& PKroot);
